...
function [r2_best, r1_best, value_best, error_best, r2_sort, r1_sort, value_sort, error_sort] = e_series_series(value, range, series, dir)
    % E series values
    e3   = [1.0 2.2 4.7 10.0];
    ...
    if isnumeric(value)
        ...
        elseif value < 0
            ...
            if numel(range) == 1
                if range > value
                    error('Individual resistors must be lower than total resistor value');
                end
            ...
            switch dir
                case 'next'
                    r2_list = e_series(value - r1_list, series, 'up');
                case 'previous'
                    r2_list = e_series(value - r1_list, series, 'down');
                case 'nearest'
                    r2_list = [e_series(value - r1_list, series, 'down') e_series(value - r1_list, series, 'up')];
                    r1_list = [r1_list r1_list];
                otherwise
                    error(['Unknown rounding command']);
            end
            % Calculate ratios and errors
            value_list = r2_list + r1_list;
