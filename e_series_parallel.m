...
%% Syntax
% 
%  r2 = e_series_ratio(value, r1_range)
%  r2 = e_series_ratio(value, r1_range, series)
%  r2 = e_series_ratio(value, r1_range, series, dir)
...
function [r2_best, r1_best, value_best, error_best, r2_sort, r1_sort, value_sort, error_sort] = e_series_parallel(value, range, series, dir)
    % E series values
    e1   = [1.0 10.0];
    e3   = [1.0 2.2 4.7 10.0];
    ...
    if isnumeric(value)
        ...
        elseif value == 0
            ...
            error_sort = 0;
        elseif value < 0
            error('Component value must be positive');
        else
            % Select E series
            if nargin <= 2
                series = 'e24';
            ...
            if numel(range) == 1
                if range < value
                    error('Individual resistors must be higher than total resistor value');
                end
                ...
                r1_list = r1_list(r1_list >= value);
            end
            % Calculate r2 vectors
            switch dir
                case 'next'
                    r2_list = e_series(1./(1/value - 1./r1_list), series, 'up');
                case 'previous'
                    r2_list = e_series(1./(1/value - 1./r1_list), series, 'down');
                case 'nearest'
                    r2_list = [e_series(1./(1/value - 1./r1_list), series, 'down') e_series(1./(1/value - 1./r1_list), series, 'up')];
                    r1_list = [r1_list r1_list];
                otherwise
                    error(['Unknown rounding command']);
            end
            % Calculate ratios and errors
            value_list = 1./(1./r2_list + 1./r1_list);
            error_list = abs(value_list - value);
            % Sort results
            [error_sort, index_sort] = sort(error_list);
            value_sort = value_list(index_sort);
            r1_sort = r1_list(index_sort);
            r2_sort = r2_list(index_sort);
            % Select best result
            r1_best = r1_sort(1);
            r2_best = r2_sort(1);
            value_best = value_sort(1);
